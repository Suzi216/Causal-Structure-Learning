function(alg_name) {
  ...
  files_G <- list.files("data/true_amat", pattern = "\\.csv$", full.names = TRUE)[order(as.numeric(sub("data/true_amat/dag_(\\d+)_.*", "\\1", list.files("data/true_amat", pattern = "\\.csv$", full.names = TRUE))))]
  
  # files_EG <- list.files( paste("gen_data", alg_name, sep = "/"), pattern = "\\.csv$", full.names = TRUE)
  files_EG <- list.files(paste("gen_data", "tabu", sep = "/"), pattern = "\\.csv$", full.names = TRUE)[order(as.numeric(sub(paste("gen_data", "/", "tabu", "/gen_dag_(\\d+)_.*", sep = ""), "\\1", list.files(paste("gen_data", "tabu", sep = "/"), pattern = "\\.csv$", full.names = TRUE))))]
  
  # files_Data <- list.files("data", pattern = "\\.csv$", full.names = TRUE)
  files_Data <- list.files("data", pattern = "\\.csv$", full.names = TRUE)[order(as.numeric(sub("data/dag_(\\d+)_.*", "\\1", list.files("data", pattern = "\\.csv$", full.names = TRUE))))]
  ...
  confusionAlg(all_results_list,substr(tools::file_path_sans_ext(basename(files_G)), 5, nchar(basename(files_G))),alg_name=alg_name, sep_con="d-sep")
  confusionAlg(all_results_listC,substr(tools::file_path_sans_ext(basename(files_EG)), 9, nchar(basename(files_G))),alg_name=alg_name,sep_con="pcor")
